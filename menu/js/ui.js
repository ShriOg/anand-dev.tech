...
const UI = (() => {
    ...
    let _customerInfo = {};
    const _minPrice = (item) => Math.min(...(item.prices || []).map(p => Number(p.value) || 0));

    const renderStats = () => {
        const items = MenuData.allItems();
        const specials = items.filter(i => i.special).length;
        const avg = items.length ? Math.round(items.reduce((s, i) => s + _minPrice(i), 0) / items.length) : 0;
        ...
        const vegStatusEl = $('#veg-status');
        if (vegStatusEl) {
            if (!items.length) {
                vegStatusEl.textContent = '';
                vegStatusEl.style.display = 'none';
            } else {
                const allVeg = items.every(i => i.veg === true);
                const allNonVeg = items.every(i => i.veg === false);

                vegStatusEl.style.display = '';
                if (allVeg) {
                    vegStatusEl.textContent = '🌱 100% Pure Veg';
                    vegStatusEl.setAttribute('aria-label', '100% Pure Veg');
                } else if (allNonVeg) {
                    vegStatusEl.textContent = '🔴 Non-Veg';
                    vegStatusEl.setAttribute('aria-label', 'Non-Veg');
                } else {
                    vegStatusEl.textContent = '🟢 Veg & 🔴 Non-Veg';
                    vegStatusEl.setAttribute('aria-label', 'Veg and Non-Veg');
                }
            }
        }
    };
    ...
    const _matchFilter = (item, filter) => {
        ...
        if (filter === 'veg') return item.veg !== false;
        if (filter === 'under50')  return _minPrice(item) < 50;
        if (filter === 'under100') return _minPrice(item) < 100;
        return true;
    };
    ...
    const renderCategoryTabs = () => {
        ...
        const makeTab = (key, label, active = false) => {
        ...
        fragment.appendChild(makeTab('all', 'All', true));
        MenuData.entries().forEach(([key, cat]) => {
            const label = `${cat.icon ? `${cat.icon} ` : ''}${cat.title || key}`;
            fragment.appendChild(makeTab(key, label, false));
        });
    ...
    const renderMenu = () => {
        ...
        cats.forEach(key => {
            ...
            html += `
            <section class="cat-section" id="cat-${key}" data-category="${key}" aria-label="${cat.title}">
                <div class="cat-head">
                ...
                <div class="menu-grid">
                    ...
                    <article class="card menu-card${item.special ? ' card--special' : ''}" style="animation-delay:${i * 0.04}s">
                        ${_minPrice(item) < 50 ? '<span class="badge-budget">Under ₹50</span>' : ''}
                        <div class="card__head">
                            <span class="${item.veg === false ? 'nonveg-badge' : 'veg-badge'}" aria-label="${item.veg === false ? 'Non Vegetarian' : 'Vegetarian'}">${item.veg === false ? '🔴 Non Veg' : '🟢 Veg'}</span>
