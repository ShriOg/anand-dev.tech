...
document.addEventListener('DOMContentLoaded', () => {
    const { $, $$, renderMenu, renderStats, renderCartBadge, renderCartModal,
            toggleCart, showToast, showSkeleton, setActiveTab, setActiveChip, renderCategoryTabs } = UI;
    ...
    let activeCategoryInView = 'all';

    const debounce = (fn, ms) => {
        let t;
        return (...args) => { clearTimeout(t); t = setTimeout(() => fn(...args), ms); };
    };
    const showAddedItemFeedback = (itemId) => {
        const name = MenuData.findById(itemId)?.name || 'Item';
        if (typeof window.showAddedItem === 'function') {
            window.showAddedItem(name);
            return;
        }
    ...
    // Backend health ping removed (server doesn't expose /stats). MenuData.connectLive handles connection state.
    ...
    const setupCategoryObserver = () => {
        ...
        const sections = Array.from(document.querySelectorAll('.cat-section[data-category]'));
        ...
        categoryObserver = new IntersectionObserver((entries) => {
            ...
            const key = best.target.dataset.category;
            if (!key || key === activeCategoryInView) return;

            activeCategoryInView = key;
            setActiveTab(key);
            const tab = document.querySelector(`.tab[data-category="${key}"]`);
            if (tab) {
    ...
    State.on('category', (v) => {
        activeCategoryInView = v;
        setActiveTab(v);
        const targetTab = document.querySelector(`.tab[data-category="${v}"]`);
    ...
    document.addEventListener('click', async (e) => {
        ...
        try {
            const res = await fetch(`${RestaurantConfig.API_URL}/orders/${id}/cancel`, { method: 'PATCH' });
    ...
    document.addEventListener('click', async (e) => {
        ...
        try {
            await fetch(`${RestaurantConfig.API_URL}/orders/${id}`, { method: 'DELETE' });
